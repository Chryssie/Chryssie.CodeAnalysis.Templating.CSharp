...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests
{
    public class LexicalTests
    {
        ...
        {
            int offset = 0;
            using (var lexer = new InternalSyntax.Lexer2(MakeSourceText(text, offset), options ?? CSharpParseOptions.Default))
            {
                var position = 0;
                while (true)
                {
            ...
            /// </summary>
            static SourceText MakeSourceText(string text, int offset)
            {
        ...
        {
            using (var lexer = new InternalSyntax.Lexer2(SourceText.From(text), _options))
            {
        ...
        private IEnumerable<InternalSyntax.BlendedNode2> Blend(string text)
        {
            using (var lexer = new InternalSyntax.Lexer2(SourceText.From(text), _options))
            {
                var blender = new InternalSyntax.Blender2(lexer, null, null);
                InternalSyntax.BlendedNode2 result;
                do
                {
